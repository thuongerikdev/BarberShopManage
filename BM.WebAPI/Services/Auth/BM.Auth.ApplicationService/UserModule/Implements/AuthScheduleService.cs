...
namespace BM.Auth.ApplicationService.UserModule.Implements
{
    public class AuthScheduleService : AuthServiceBase, IAuthScheduleService
    {
        ...
        public async Task<ResponeDto> GetEmployeesByDateAndBranch(DateTime date, int branchesID , string typeOfEmp)
        {
            ...
            try
            {
                var employeeSchedules = await _dbContext.ScheEmps
                    .Where(se => se.startDate.HasValue
                                && se.endDate.HasValue
                                && EF.Functions.DateFromParts(se.startDate.Value.Year, se.startDate.Value.Month, se.startDate.Value.Day) <= date.Date
                                && EF.Functions.DateFromParts(se.endDate.Value.Year, se.endDate.Value.Month, se.endDate.Value.Day) >= date.Date
                                && se.status == "ASSIGNED"
                                && se.AuthEmp.AuthPosition.positionName == typeOfEmp
                                && se.AuthEmp.branchID == branchesID)
                    ...
                    .Select(se => new
                    {
                        ...
                        image = se.AuthEmp.AuthUser.avatar,
